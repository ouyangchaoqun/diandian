    ...
    <div class="friendList_box">
        <div v-if="hasFriend!=null&& hasFriend==true">
            <div class="weui-cells friendsCount_box specialCare_box" v-if="friend_s.length">
                <div class="friendsCount_cell specialCare"><img src="../images/mafriend_icom_special.png" alt="">特别关心</div>
            ...
            <div class="weui-cells friendsCount_box">
                <div v-for="(user,index) in friend_g"  :key="index">
                    ...
                    <div class="friendsCount_cell" v-if="index>0&& friend_g[index-1].firstCn[0]!=friend_g[index].firstCn[0]">{{friend_g[index].firstCn[0]}}</div>
...
<style>
    ...
    .friendsCount_box div .weui-cell:first-child:before{ display: block}
    ...
    .bgw{ background: #fff;padding-bottom: 25px;}
    .btn-gray{ background: #ebeceb; line-height: 30px; text-align: center; margin-left: 15px !important; margin-right: 15px !important;
        border-radius: 5px;    border-color: #ebeceb; overflow: hidden}
    .btn-gray a{-webkit-tap-highlight-color: rgba(0,0,0,.2) !important;  padding: 0 !important; display: inherit;; background: none;font-size: 13px;  color:#5d5d5d}

    .bottom-card{background: #ffffff;padding:7px 0 59px 0 ; padding-left: 15px; padding-right: 15px}
    .bottom-card  .weui-btn{ -webkit-tap-highlight-color: rgba(0,0,0,.2) !important;}
</style>
        ...
        mounted: function () {
            ...
            this.$http.get(web.API_PATH + 'user/query/friend/by/user/id/_userId_' ).then(function (data) {
                    ...
                    for(let i =0;i<_this.friend_g.length;i++){
                        ...
                        _this.friend_g[i].friendLink=   "/#/friendCenter/?friendId="+_this.friend_g[i].id;
                    }
